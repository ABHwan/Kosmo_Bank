...
public class CustomerController {
	...
	private IamportClient api = new IamportClient("imp_apikey",
	...
	public String certifications(HttpServletRequest req, HttpSession session, Model model) throws Exception {
		...
		String urlstr = "https://api.iamport.kr/certifications/" + imp_uid + "?_token=" + access_token; // 클라이언트가 요청을 보낼
																										// 서버의 URL 주소
	...
	// 회원 공과금 - 아파트 관리비(유성)
	@RequestMapping("apt_maintenance")
	public String apt_maintenance(HttpServletRequest req, Model model) {
		System.out.println("url => apt_maintenance");
		logger.info("url ==> apt_maintenance");

		service.getAccount(req, model);
		service.aptFee(req, model);

		return "customer/utility_bill/apt_maintenance";
	}

	// 회원 공과금 - 등록금(유성)
	@RequestMapping("tuition")
	public String tuition(HttpServletRequest req, Model model) {
		System.out.println("url => tuition");
		logger.info("url ==> tuition");

		service.getAccount(req, model);
		service.tuitionFee(req, model);

		return "customer/utility_bill/tuition";
	}

	// 회원 공과금 - 지방세(유성)
	@RequestMapping("localTex")
	public String localTex(HttpServletRequest req, Model model) {
		System.out.println("url => localTex");
		logger.info("url ==> localTex");

		service.getAccount(req, model);
		service.localFee(req, model);
