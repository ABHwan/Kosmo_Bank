					...
					<div class="page-inner">
						...
						<div class="page-header">
							...
							<ul class="breadcrumbs">
								<li class="nav-home"><a href="${custPath}index.do"> <i
										class="flaticon-home"></i>
								</a></li>
								<li class="separator"><i class="flaticon-right-arrow"></i>
								</li>
								<li class="nav-item"><a href="${custPath}accountConnect">계좌연동</a>
								</li>
								<li class="separator"><i class="flaticon-right-arrow"></i>
								</li>
											...
											<c:forEach var="i" begin="0" end="5" varStatus="status">
												...
												<c:forEach var="j" begin="0" end="5" varStatus="statusJ">
													<c:if test="${dtos[j].account_bankCode == status.count}">
														...
														<tr>
															<td class="bank-td"><c:choose>
																	<c:when test="${status.count == 1}">
																		<img class="bankImg"
																			src="${rePath}images/bank/gookminicon.jpg">
																		<span class="bankname">국민은행</span>
																	</c:when>
																	<c:when test="${status.count == 2}">
																		<img class="bankImg"
																			src="${rePath}images/bank/uriicon.jpg">
																		<span class="bankname">우리은행</span>
																	</c:when>
																	<c:when test="${status.count == 3}">
																		<img class="bankImg"
																			src="${rePath}images/bank/nhicon.png">
																		<span class="bankname">농협은행</span>
																	</c:when>
																	<c:when test="${status.count == 4}">
																		<img class="bankImg"
																			src="${rePath}images/bank/sinhanicon.png">
																		<span class="bankname">신한은행</span>
																	</c:when>
																	<c:when test="${status.count == 5}">
																		<img class="bankImg"
																			src="${rePath}images/bank/hanaicon.png">
																		<span class="bankname">하나은행</span>
																	</c:when>
																	<c:otherwise>
																		<img class="bankImg"
																			src="${rePath}images/bank/kosmoicon.png">
																		<span class="bankname">코스모은행</span>
																	</c:otherwise>
																</c:choose></td>
															<td class="btn-td"><label for="account_bankCode${j}"
																class="account-label"> <c:if
																		test="${dtos[j].account_linked == 0 }">
																		<button
																		...
																		<input type="button" class="submit-btn"
																			id="account_bankCode${j}" name="account_bankCode"
																			value="${dtos[j].account_bankCode}"
																			onclick="accountConnectConfirm(${j});">

																	</c:if>
															</label> <c:if test="${dtos[j].account_linked == 1}">
																	<button type="button"
													...
													<c:if test="${flag == 0 && j == 5}">
														<tr>
															...
															<td class="btn-td">
																...
																</button> <small class="account-null">계좌없음</small>
															</td>
														</tr>
													</c:if>
												</c:forEach>
